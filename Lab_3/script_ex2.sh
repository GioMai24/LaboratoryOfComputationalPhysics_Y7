...
even=0
for i in $(grep -oE "[0-9]+" data.txt); do if ((  $i % 2 == 0 )); then ((even++)); fi; done; echo $even
...
for i in $(grep -oE "[1-9]+" < data.txt); do
	if [ $(echo "$i < $thr" | bc -l) -eq 1 ]; then les+=("$i"); else gre+=("$i"); fi
done
...
for i in $(seq 1 $1); do echo "$(grep -oE "[1-9]+")/$i" | bc -l > data_$i.txt; done
